...
function start_tool(wordsreceived) {
  ...
  if (words != "") {
    ...
    var sedata = {
      ...
      prompt:
        "For this given word after three ! marks at the end of this promot. display an example of the word used in a sentence in the respective language. output JUST the example and not a single extra word. Don't halucinate. !!!",
    };
    $.ajax({
...
$("#1").click(function alter_rank_to1() {
  ...
  $.ajax({
    url: "https://vibereader.onrender.com/alter",
    type: "POST",
    contentType: "application/json",
...
vocab.addEventListener("click", function () {
  $.ajax({
    url: "https://vibereader.onrender.com/start",
    success: function (response) {
