    ...
    if (answer.innerText == "") {
      $.ajax({
        ...
        success: function (response) {
          ...
          $.ajax({
            url: "https://vibereader.onrender.com/receive_data",
            type: "POST",
            contentType: "application/json",
...
$("#start").on("click", function () {
  ...
  document.addEventListener("mouseup", async () => {
    ...
    sdata = {
      word: `${selectedText}`,
      prompt:
        "Translate the following sentences to this Prompt from the language they are most probably are to English output nothing but just the translation and nothing more not a single word more.",
    };
    if (selectedText != "") {
      $.ajax({
        url: `https://vibereader.onrender.com/proompts`,
